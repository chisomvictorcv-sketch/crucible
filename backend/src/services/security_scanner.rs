...
impl SecurityScanner {
    ...
    pub async fn scan(
        ...
        let has_auth = source_code.contains("require_auth") || source_code.contains("require_auth_for_args");
        if has_transfer && !has_auth {
            findings.push(SecurityFinding {
                id: Uuid::new_v4().to_string(),
                severity: "medium".to_string(),
                title: "Missing authorization check".to_string(),
                description: "Sensitive operations (transfer/mint) found without require_auth checks.".to_string(),
                line_number: None,
                recommendation: "Add require_auth() or require_auth_for_args() before sensitive operations.".to_string(),
            });
        }
        ...
        let has_checked = source_code.contains("checked_add") || source_code.contains("checked_mul") || source_code.contains("saturating_");
        if has_arithmetic && !has_checked {
            findings.push(SecurityFinding {
                id: Uuid::new_v4().to_string(),
                severity: "medium".to_string(),
                title: "Potential integer overflow".to_string(),
                description: "Arithmetic operations found without checked or saturating variants.".to_string(),
                line_number: None,
                recommendation: "Use checked_add, checked_mul, or saturating_add to prevent overflow.".to_string(),
            });
        }
        ...
        if source_code.contains("G") && source_code.contains("AAAA") {
            findings.push(SecurityFinding {
                ...
                description: "A Stellar address appears to be hardcoded in the contract source.".to_string(),
                line_number: None,
                recommendation: "Store addresses in contract storage or pass them as parameters.".to_string(),
            });
        }

        let risk_score: f64 = findings.iter().map(|f| match f.severity.as_str() {
            "critical" => 25.0,
            "high" => 15.0,
            "medium" => 5.0,
            "low" => 1.0,
